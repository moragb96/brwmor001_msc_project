import IPython,casperfpga,time,struct,os,sys,logging,pylab,random,math,re,argparse,shutil
import numpy as np
...
def parse_input_file(inputs_path, input_file, fft_type, nfft, sr, sr_type, demux):
        ...
        output_file = "./outputs/{}in/au50_{}_{}_".format(str(demux), fft_type, nfft) + input_file + "_sr-{}".format(sr_type) + ".output"
...
def write_signal(sig, demux, n):
        ...
        for j in range(demux):
            for jj in range(n//demux):                                                    # n/demux is number of samples per bram
                buf['buf{}'.format(j)] = struct.pack('>1i', d['d{}'.format(j)][jj])    
                fpga.write('bram_in{}'.format(j), buf['buf{}'.format(j)], 4*jj)
...
def read_input_brams(demux, n, plot = True):
        ...
        b = {}
        for i in range(demux):
            b['in{}'.format(i)] = np.array(struct.unpack('>{}i'.format(str(n//demux)), fpga.read('bram_in{}'.format(i) , (n//demux)*4 , 0)), dtype = float)
        ...
        return np.array(interleave_in, dtype=float)
...
def read_input_ss(demux, plot = True):
        ...
        return np.array(ss_sig, dtype=float)
...
def get_output_data(demux, n, fft, output_file, fs, plot = True):
        ...
        if fft == 'casper':
            ...
            interleave_re_array = np.array(interleave_re,dtype=float)
            interleave_im_array = np.array(interleave_im,dtype=float)
        ...
        # astron outputs only pos freq channels, but as re0 = [a0, b0, a1, b1 ...] etc
            ...
            if demux == 4:
                for i in range(n//demux):
                    for ii in reversed(range(demux)):
                        interleave_re.append(re_out['re{}'.format(ii)][i])
                        interleave_im.append(im_out['im{}'.format(ii)][i])
                ...
                for i in range(n//demux):
                    for ii in reversed(range(demux)):
                        if ii < demux//2:
                            ...
                            interleave_re.append(re_out1['re{}'.format(ii)][i])
                            interleave_im.append(im_out1['im{}'.format(ii)][i])

            interleave_re_array = np.array(interleave_re,dtype=float)[0:n//2]
            interleave_im_array = np.array(interleave_im,dtype=float)[0:n//2]

        complex_vals = interleave_re_array + interleave_im_array*1j    
        complex_out = abs(interleave_re_array + interleave_im_array*1j)

        if plot:
            f_axis = np.arange(0,n//2)*fs/n
            plt.plot(f_axis, complex_out[0:n//2])
            plt.title("AU50: {} FFT, {} points".format(fft.upper(), str(n)))
            plt.xlabel("Freq (MHz)")
            plt.show()
...
if __name__ == '__main__':
        ...
        p.add_argument('--board', dest='board', type=int, action='store', default='', help='hardware platform hostname or port')
        ...
        # get current working directory path for fpg file copying
        cwd = os.getcwd()

        # connect to board
        logging.info('Connecting to AU50')
        fpga = casperfpga.CasperFpga('10.8.96.160', port = args.board)
        time.sleep(0.2)

        if fpga.is_connected():
            logging.info('Connected to AU50')
        else:
            logging.info('ERROR connecting to AU50')
            exit_fail()
        ...
        for input_file in input_files:
            ...
            # must copy fpg to script directory cos alveo transport doesn't like paths - fpg removed from script directory after
            for fpg_file in fpg_files:
                logging.info('---------------------------------------------')
                logging.info('Using fpg file: {}'.format(fpg_file))
                fft_type, nfft = parse_fpg_file(fpg_file)
    
                # copy fpg file to cwd
                shutil.copy(join(args.fpgs, fpg_file), cwd)
    
                # program board and load fpg information 
                logging.info('Programming AU50')
                try:
                    fpga.transport.upload_to_ram_and_program(fpg_file)
                except:
                    IPython.embed()
                fpga.get_system_information(fpg_file)
                logging.info('Done programming AU50')
                ...
                # shift_type = ['alt']
                #shift_dict = {2**10: [682], 2**14: [10922], 2**16: [43690]}
                shift_type = ['none', 'alt']
                ...
                for isr, sr in enumerate(shift_schedules):
                    ...
                    if demux == 8 and (fft_type == 'astron' or fft_type == 'xilinx'):
                ...
                # remove fpg file from cwd for neatness once done
                os.remove(join(cwd, fpg_file))
